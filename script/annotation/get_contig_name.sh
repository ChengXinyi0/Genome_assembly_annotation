...
#SBATCH --job-name=contig_name
#SBATCH --mail-user=xinyi.cheng@students.unibe.ch
#SBATCH --mail-type=begin,end
#SBATCH --output=/data/users/xcheng/assembly_annotation_course/annotation/logs/contig_name_%j.o
#SBATCH --error=/data/users/xcheng/assembly_annotation_course/annotation/logs/contig_name_%j.e

module load UHTS/Analysis/SeqKit/0.13.2
# got all contig names
fastafile=/data/users/xcheng/assembly_annotation_course/annotation/flye_polished1.fasta
cd /data/users/xcheng/assembly_annotation_course/annotation
seqkit seq --quiet --name $fastafile > contig_names.txt

# 1. compute the length of each contig and save in a list
awk -F "\t" -v contig_file="/data/users/xcheng/assembly_annotation_course/annotation/contig_names.txt" '
    BEGIN {
...
contig_length=/data/users/xcheng/assembly_annotation_course/annotation/logs/find_10_contig_11938638.o

sort -k2,2nr $contig_length | head -n 10 | awk '{print $1}' > /data/users/xcheng/assembly_annotation_course/annotation/result/accession/top_10_ids.txt

# 2. Create a bed file with the position of every gene found in these contigs
ids=/data/users/xcheng/assembly_annotation_course/annotation/result/accession/top_10_ids.txt
...
cd /data/users/xcheng/assembly_annotation_course/annotation/result/accession
awk '{print $4}' Cvi0.bed > Cvi0_gene_ids.txt

# 3.2 Fetch their protein sequences from the MAKER output
cd /data/users/xcheng/assembly_annotation_course/annotation/result
